...
function App() {
  ...
  useEffect(() => {
    async function getItems() {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URI}note/`);
  ...
  async function passInfo(titleSubmitted, contentSubmitted) {
    ...
    await fetch(`${process.env.REACT_APP_BACKEND_URI}note/add`, {
      method: "POST",
      headers: {
  ...
  async function deleteItem(id) {
    ...
    await fetch(`${process.env.REACT_APP_BACKEND_URI}${id}`, {
     method: "DELETE"
   });
  //  navigate("/loading");
  }
